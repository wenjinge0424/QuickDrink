...
-(void)layoutSubviews{
    ...
    CGRect  newRect = [self convertRect:self.frame toView:self.superview.superview];
    tblView = [[UITableView alloc] initWithFrame:CGRectMake(newRect.origin.x, newRect.origin.y, self.frame.size.width, self.frame.size.height)] ;
...
-(void)didTap : (UIGestureRecognizer *)gesture {
    ...
    if(!isCollapsed) {
        ...
        [self.superview.superview addSubview:tblView];
        [tblView reloadData];
        
        [UIView animateWithDuration:0.25 animations:^{
            CGRect  newRect = [self convertRect:self.frame toView:self.superview.superview];
            if(_DirectionDown)
                tblView.frame = CGRectMake(newRect.origin.x, newRect.origin.y + self.frame.size.height+5, self.frame.size.width, height);
            else
                tblView.frame = CGRectMake(newRect.origin.x, newRect.origin.y - 5 - height, self.frame.size.width, height);
        }];
        ...
        view.tag = 99121;
        [self.superview.superview insertSubview:view belowSubview:tblView];
...
-(void)collapseTableView{
    if(isCollapsed){
        [UIView animateWithDuration:0.25 animations:^{
            CGRect  newRect = [self convertRect:self.frame toView:self.superview.superview];
            if(_DirectionDown)
                tblView.frame = CGRectMake(newRect.origin.x, newRect.origin.y + self.frame.size.height+5, self.frame.size.width, 0);
            else
                tblView.frame = CGRectMake(newRect.origin.x, newRect.origin.y, self.frame.size.width, 0);
        }];
        
        [[self.superview.superview viewWithTag:99121] removeFromSuperview];
