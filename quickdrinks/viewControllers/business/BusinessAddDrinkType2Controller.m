//
//  BusinessAddDrinkType2Controller.m
//  quickdrinks
//
...
@interface BusinessAddDrinkType2Controller ()<UITextFieldDelegate, UITextViewDelegate, UIImagePickerControllerDelegate,UINavigationControllerDelegate, CustomComboBox_SelectedItem>
{
    int animationHeight;
    
    int selected_index;
    int selected_brand_index;
...
@property (strong, nonatomic) IBOutlet CustomComboBox *combo_catgory;
@property (strong, nonatomic) IBOutlet CustomComboBox *combo_brand;
@property (weak, nonatomic) IBOutlet UITextField *edt_brandName;
...
@implementation BusinessAddDrinkType2Controller
...
- (void)viewDidLoad {
    ...
    UIBarButtonItem * done3Button = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(onHideKeyboardForInstruction)];
    ...
    [toolbar3 sizeToFit];
    NSArray * items3Array = [NSArray arrayWithObjects:flexButton,done3Button, nil];
    [toolbar3 setItems:items3Array];
    [self.edt_instruction setInputAccessoryView:toolbar3];
    
    [self.checkerGroup initViewDelegate];
    [self.checkerGroup setSelectItem:0];
    ...
    selected_index = 0;
    self.combo_catgory.lbl_title.text = @"Brandy";
    self.combo_catgory.m_containDatas = [[NSMutableArray alloc] initWithObjects:@"Brandy",@"Cognac",@"Schnapps",@"Gin",@"Rum",@"Tequila",@"Vodka",@"Whisky",@"Bourbon", @"Scotch",nil];
    self.combo_catgory.delegate = self;
    
    selected_brand_index = 0;
    self.combo_brand.lbl_title.text = @"Bloody Mary";
    self.combo_brand.m_containDatas = [[NSMutableArray alloc] initWithObjects:@"Bloody Mary", @"Cosmopolitan", @"Martini", nil];
    self.combo_brand.delegate = self;
    ...
    [self addTextField:self.edt_brandName withCheckFunction:^BOOL(NSString* str){
...
//        }
        ...
        return YES;
    } withSuccessAction:^(){[Util setBorderView:self.edt_brandName.superview color:[UIColor clearColor] width:1.f];
    } withFailAction:^{ [Util setBorderView:self.edt_brandName.superview color:[UIColor redColor] width:1.f];
    } withDefaultAction:^{[Util setBorderView:self.edt_brandName.superview color:[UIColor clearColor] width:0.f];}];
    
    [self addTextField:self.edt_nameOfBeer withCheckFunction:^BOOL(NSString* str){
        if(str.length == 0 || str.length>50 || str.length<3){
            return NO;
        }
//        NSCharacterSet * set = [[NSCharacterSet characterSetWithCharactersInString:@"abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLKMNOPQRSTUVWXYZ "] invertedSet];
//        if ([str rangeOfCharacterFromSet:set].location != NSNotFound){
...
//        }
    ...
    [self addActionField:self.edt_instruction withCheckFunction:^BOOL(UITextView* item){
        NSLog(@"%@", item.text);
        if(item.text.length > 1000){
            return NO;
        }
        return YES;
    } withSuccessAction:^(){[Util setBorderView:self.edt_instruction.superview color:[UIColor clearColor] width:1.f];
    } withFailAction:^{ [Util setBorderView:self.edt_instruction.superview color:[UIColor redColor] width:1.f];
    } withDefaultAction:^{[Util setBorderView:self.edt_instruction.superview color:[UIColor clearColor] width:0.f];}];
...
- (void) setEditMode
{
    ...
    [self.edt_instruction setText:currentItem.row_instruction];
    ...
    self.combo_catgory.lbl_title.text = [self.combo_catgory.m_containDatas objectAtIndex:currentItem.row_subcategory_id];
    self.combo_brand.lbl_title.text = [self.combo_brand.m_containDatas objectAtIndex:currentItem.row_selected_type];
    self.edt_brandName.text = currentItem.row_selected_typeName;
...
- (void) onHideKeyboardForInstruction
{
...
- (IBAction)onCheck:(id)sender {
    ...
    [self onDone:^{
        ...
        if(!self.isEditMode){
            ...
            item.row_category_id = 2;
            item.row_subcategory_id = selected_index;
            item.row_selected_type = selected_brand_index;
            item.row_selected_typeName = self.edt_brandName.text;
            item.row_name = trimmedString;
            item.row_price = [number floatValue];
            item.row_description = self.edt_description.text;
            item.row_instruction = self.edt_instruction.text;
            [SVProgressHUD showWithMaskType:SVProgressHUDMaskTypeGradient];
           ...
           /* }];*/
            ...
            currentItem.row_category_id = 2;
            currentItem.row_subcategory_id = selected_index;
            currentItem.row_selected_type = selected_brand_index;
            currentItem.row_selected_typeName = self.edt_brandName.text;
            ...
            currentItem.row_instruction = self.edt_instruction.text;
            [SVProgressHUD showWithMaskType:SVProgressHUDMaskTypeGradient];
        ...
        [Util showAlertTitle:self title:@"Quick Drink" message:message finish:^{
            if(targetView){
                dispatch_async(dispatch_get_main_queue(), ^{
                    if(targetView == self.edt_brandName){
                        [self.edt_brandName becomeFirstResponder];
                    }else if(targetView == self.edt_nameOfBeer){
                        [self.edt_nameOfBeer becomeFirstResponder];
                    ...
                    }else if(targetView == self.edt_instruction){
                        [self.edt_instruction becomeFirstResponder];
...
/////
- (int) getIndexFromStringArrayforString:(NSString*)str :(CustomComboBox*)combo
{
    NSMutableArray *  array = combo.m_containDatas;
...
{
    if(combo == self.combo_catgory){
        selected_index = [self getIndexFromStringArrayforString:item :self.combo_catgory];
    }else{
        selected_brand_index = [self getIndexFromStringArrayforString:item :self.combo_brand];
    }
}
